...
def standardize_songs_artists():
    ...
    print(f"  • concert_setlists.csv: {len(concert_setlists_df)}개 항목")
    
    # 2. setlist_title → concert_title → artist 매핑 생성
    print("\n2. 매핑 테이블 생성...")
    
    # concert_setlists.csv에서 concert_title과 setlist_title 매핑
    concert_to_setlist = {}
    setlist_to_concert = {}
    for _, row in concert_setlists_df.iterrows():
        concert_title = row['concert_title']
        setlist_title = row['setlist_title']
        setlist_to_concert[setlist_title] = concert_title
        if concert_title not in concert_to_setlist:
            concert_to_setlist[concert_title] = []
        concert_to_setlist[concert_title].append(setlist_title)
    ...
    print(f"  • song → setlist 매핑: {len(song_to_setlist)}개")
    ...
    for original_artist in original_artists:
        ...
        if found_artists:
            ...
            not_found_count += 1
            # print(f"  ❌ '{original_artist}' 매핑 찾을 수 없음")
    
    print(f"\n  • 매핑 생성 완료: {updated_count}개 변경, {not_found_count}개 유지")
    ...
    if len(matched) < len(new_artists):
        ...
        print(f"\n  ⚠️ 매칭되지 않은 artist ({len(unmatched)}개):")
        for i, artist in enumerate(sorted(unmatched)[:10], 1):
            print(f"     {i}. {artist}")
        if len(unmatched) > 10:
            print(f"     ... 외 {len(unmatched) - 10}개")
    
    print("\n" + "=" * 60)
    print("✅ Artist 표준화 작업 완료")
    print("=" * 60)
